...
float dequantize_unorm(uint bits, uint value) { return float(value) / float((1 << bits) - 1); }
float dequantize_snorm(uint bits, int value) { return float(value) / float((1 << (bits - 1)) - 1); }
float dequantize_snorm(int16_t value) { return dequantize_snorm(16, int(value)); }
vec3 dequantize_snorm(int16_t values[3]) {
    return vec3(
        dequantize_snorm(values[0]), dequantize_snorm(values[1]), dequantize_snorm(values[2])
    );
}
...
TangentFrame decode_tangent_frame(uint encoded) {
    ...
    vec2 uv = vec2(
        dequantize_unorm(10, encoded & UV_MASK), dequantize_unorm(10, (encoded >> 10) & UV_MASK)
    );
    ...
    tangent_frame.tangent =
        orthonormal * cos(angle) + cross(tangent_frame.normal, orthonormal) * sin(angle);
